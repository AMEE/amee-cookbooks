...
#
# I presume it needs 1.8.7 to bootstrap 1.9, but to be honest, I'm
# cargo-culting a lot of this

# we need curl to download the archive (I think)
...
# rvm-install 1.8.7 - not if it's already there (these builds take a long time)
...
# then install rails (we do this via a bash line because gem_install seems to
# use the system ruby or something. It's a hack, but it works)
bash "install rails" do
  code "gem install rails --version #{node[:rails][:version]}"
end

# something something something rvm. The rails binary does not go somewhere
# sensible like /usr/local/bin, so we use a symlink (could not get
# update-alternatives to play nice)
